...
class PSM_RAKERNIS_ViewSet(viewsets.ModelViewSet):
    queryset = models.PSM_RAKERNIS.objects.all().filter(satker__parent__isnull=True).order_by('satker__id','-tanggal_awal', 'satker__order').distinct('satker__id')
    ...
    def kirim_kegiatan(self, request):
        ...
        try:
            ...
            if user_satker_level == 1:
                ...
                satker_parent['keterangan'] = user_nama_satker_parent
...
class PSM_BINAAN_TEKNIS_ViewSet(viewsets.ModelViewSet):
    queryset = models.PSM_BINAAN_TEKNIS.objects.all().filter(satker__parent__isnull=True).order_by('satker__id','-tanggal_awal', 'satker__order').distinct('satker__id')
...
class PSM_TES_URINE_DETEKSI_DINI_ViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if satker_level == 1:
            queryset = queryset.filter(satker_id=satker)
        elif satker_level == 0:
            queryset = queryset.filter(satker__provinsi_id=satker_provinsi, status__gt=0)
        elif satker_level == 2:
            queryset = queryset.filter(status=2)
...
class PSM_MONITORING_DAN_EVALUASI_SUPERVISI_CURD_ViewSet(viewsets.ModelViewSet):
    ...
    def perform_update(self, serializer):
        ...
        models.PSM_MONITORING_DAN_EVALUASI_SUPERVISI.objects.filter(parent_id=current_record_id).delete()
        ...
        if peserta_array_lama_json:
            ...
            if len(data) > 0:
                ...
                for entry in data:
                    ...
                    peserta = models.PSM_MONITORING_DAN_EVALUASI_SUPERVISI.objects.create(
                        parent=parent_instance,
                        nama_peserta=entry['nama'],
                        jenis_kelamin=jenis_kelamin,
                        jabatan=entry['jabatan']
...
class PSM_AUDIENSI_ViewSet(viewsets.ModelViewSet):
    queryset = models.PSM_AUDIENSI.objects.all().filter(satker__parent__isnull=True).order_by('satker__id','-tanggal_awal', 'satker__order').distinct('satker__id')
