...
class PSM_TES_URINE_DETEKSI_DINI_ViewSet(viewsets.ModelViewSet):
    ...
    def get_data_peserta(self, parent):
        data = models.PSM_TES_URINE_DETEKSI_DINI_PESERTA.objects.filter(parent=parent).values(
            'id', 'nama_peserta', 'jenis_kelamin', 'hasil_test', 'alamat', 'isi_parameter'
        ).order_by('-id')
...
class PSM_KEGIATAN_LAINNYA_ViewSet(viewsets.ModelViewSet):
    ...
    def get_detail_data(self, request):
        ...
        data = models.PSM_KEGIATAN_LAINNYA.objects.values(
            'id', 'satker_id', 'satker__nama_satker', 'kegiatan', 'tempat', 'waktu_awal', 'waktu_akhir', 'lingkungan', 'jumlah_sasaran', 'hasil_dampak', 'kesimpulan', 'tindak_lanjut', 'dokumentasi', 'status', 'satker_id__level'
        )
...
class PSM_BIMTEK_P4GN_ViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, request):
        ...
        if serializer.is_valid():
            ...
            print(self.request.data.get('peserta_bimtek'))
            if peserta_bimtek:
    ...
    def get_detail_data(self, request):
        ...
        data = models.PSM_BIMTEK_P4GN.objects.values(
            'id', 'satker_id', 'peserta', 'nama_lingkungan', 'seri_pin_penggiat', 'satker__nama_satker', 'tanggal_awal', 'tanggal_akhir',
            'hasil_capaian', 'kendala', 'kesimpulan', 'tindak_lanjut', 'dokumentasi', 'status'
        ).order_by('satker__nama_satker').filter(id=id)
    ...
    def get_data_peserta(self, parent):
        data = models.PSM_BIMTEK_P4GN_PESERTA.objects.filter(parent=parent).values(
            'id', 'nama', 'jenis_kelamin', 'alamat','no_telepon'
        ).order_by('-id')
